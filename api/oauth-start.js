// /api/oauth-start — Server-side PKCE (Amy's mobile-safe solution)
export default async function handler(req, res) {
    if (req.method !== 'POST') return res.status(405).end();

    const clientId = process.env.DERIV_CLIENT_ID || "33ByqD0GecGTE5whirko8";

    // Detect which domain made the request and use matching redirect URI
    // Both must be registered in your Deriv app dashboard
    const origin = req.headers.origin || req.headers.referer || '';
    const redirectUri = origin.includes('btraderhub.com')
        ? 'https://btraderhub.com/callback'
        : 'https://btraderhub.vercel.app/callback';
